...
def df_to_source_target(df, args):
    ...
    sc_format_strings = [
        ...
        '[situation] situation [rot] rot [situation_attr] ~ <moral> <rot-agree> <violation-severity>', # attribute prediction
        '[situation] situation [situation_attr] <moral> <rot-agree> <violation-severity> ~ [rot] rot', # controlled generation
        '[situation] situation [rot] rot [situation_attr] ~ <moral> <rot-agree> <violation-severity>', # attribute labeling
        '[situation] situation [situation_attr] ~ <moral> <rot-agree> <violation-severity> [rot] rot', # model choice generation
    ]    
...
def main():
    ...
    OUT_DIR = os.path.join(args.output, f"{args.model_checkpoint.split('/')[-1]}_all_objectives_epochs{args.epochs}_batch{args.batchsize}")
    ...
    for s in ['train', 'dev', 'test']:
        shuffle = lambda df: df if s=='test' else df.sample(frac=1, random_state=args.seed)
        if (s=='train') and (args.train_size > 0):
            shuffle(df_to_source_target(df[df['split']==s].copy(), args)).sample(n=args.train_size, random_state=args.seed).to_csv(os.path.join(OUT_DIR,'tmp/%s.csv' % s), index=False)
        else:
            shuffle(df_to_source_target(df[df['split']==s].copy(), args)).to_csv(os.path.join(OUT_DIR,'tmp/%s.csv' % s), index=False)
